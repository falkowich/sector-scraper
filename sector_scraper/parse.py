...
def parse_temperature(filename: str) -> List[Dict[str, str]]:
    ...
    for container in area_containers:
        ...
        for tile in tiles:
            temperature = extract_digits(tile.find("div", class_="qa-place-average-temperature").text)
            place_name = tile.find("h3", class_="qa-place-name").text
            temperature_lst.append({ "area_name": area_name, "room": place_name, "temperature": temperature })
